...
parameters {
  // Type 2 SDT hyperparameters
  real mu_c2;                       // hyperparameter for c2
  real<lower=0> lambda_c2;           // hyperparameter for c2

  real mu_logMratio;                // hyperparameter for logMratio
  real<lower=0> lambda_logMratio;     // hyperparameter for logMratio

  // Type 2 SDT parameters
  vector[N] logMratio;            // log(meta-d/d)
...
transformed parameters{
  ...
  vector[N] Mratio;               // meta-d/d
  real sigma_c2;
  real sigma_logMratio;
  vector[N] meta_d;               // meta-d' actually..
  ...
  sigma_c2 = inv_sqrt(lambda_c2);
  sigma_logMratio = inv_sqrt(lambda_logMratio);
  ...
  for (i in 1:N){
    ...
    for (j in 1:(nratings-1)) {
      cS1_raw[i, j] = sample_from_tnormal_ub(c1[i], -mu_c2, lambda_c2, cS1_p[i, j]); //sampling from truncated normal distribution (upper bound)
      cS2_raw[i, j] = sample_from_tnormal_lb(c1[i], mu_c2, lambda_c2, cS2_p[i, j]); //sampling from truncated normal distribution (lower bound)
    }
    ...
    Mratio[i] = exp(logMratio[i]);
    meta_d[i] = Mratio[i] * d1[i];
...
model {
  ...
  // Type 2 priors
  mu_c2 ~ normal(0,0.01);       //
  lambda_c2 ~ gamma(0.001, 0.001);
  mu_logMratio ~ normal(0,1);
  lambda_logMratio ~ gamma(0.001, 0.001);

  logMratio ~ normal(mu_logMratio, lambda_logMratio);

  for (i in 1:N){
    for (j in 1:(nratings - 1)) {
      cS1_p[i, j] ~ uniform(0, 1);                      // sampling c1 vectors for S1 (cS1_p -> cS1_raw -> cS1)
      cS2_p[i, j] ~ uniform(0, 1);                      // sampling c2 vectors for S2 (cS2_p -> cS2_raw -> cS2)
    }
  }
